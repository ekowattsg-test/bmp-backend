...
CREATE OR REPLACE VIEW project_manpower_view AS
 SELECT p.project_code,
   ...
   FROM project_manpower m,
    ...
    project p,
    staff f
  WHERE p.project_code::text = s.project_code::text AND t.project_stream_id = s.project_stream_id AND m.project_task_id = t.project_task_id AND m.staff_id::text = f.staff_id::text
