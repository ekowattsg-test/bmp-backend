...
```
GET    /api/projectunits/project/{projectCode}
...
3. Place each unit under its parent storey and attach `projectStackId` / `stackName` so the frontend knows which column it belongs to
