...
- When generating DOs, the backend resolves `customerId` from the project record and writes it to the DO header.
- The backend aggregates duplicate products within the same DO group (same `projectCode + deliveryDate`) into a single line item with summed quantity.
- The backend computes the delivered item price from purchase-order history:
  - Average unit cost is calculated from non-cancelled `PurchaseOrderItem` rows matching the product.
  - Margin is applied from `param.deliveryPriceMargin` (percentage, e.g. `10` means +10%).
  - Resulting `unitPrice`, `lineTotal`, and DO header `deliveryAmount` are stored on the generated DO.
  - If no purchase-order history exists, `unitPrice`/`lineTotal` will be blank and `deliveryAmount` will be 0.
- The frontend does **not** need to send prices; only send `taskDeliveryRequirementId`, `deliveryQuantity`, `deliveryDate`, and `selected`.
