...
All endpoints require authentication. The backend resolves the current user's staff identity from the provided `mobileNumber` query parameter, which must belong to the authenticated user.

Base path: `/api/messages`

> **Rule:** GET endpoints must use query parameters only. Only `POST /api/messages` may use a JSON request body.

### 1. List conversation summaries

```http
GET /api/messages/conversations?mobileNumber=<mobileNumber>
```
...
```http
GET /api/messages/direct?mobileNumber=<mobileNumber>&staffId=<otherStaffId>
```
...
```http
GET /api/messages/project?mobileNumber=<mobileNumber>&projectCode=<projectCode>
```
...
```http
GET /api/messages/broadcast?mobileNumber=<mobileNumber>
```
...
```http
GET /api/messages/unread-count?mobileNumber=<mobileNumber>
```
...
```http
POST /api/messages?mobileNumber=<mobileNumber>
...
```http
PUT /api/messages/{messageId}/read?mobileNumber=<mobileNumber>
```
